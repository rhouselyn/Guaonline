            ...
            {word.examples.map((example, index) => (
              <div key={index} className="border-l-2 border-amber-200 pl-2.5">
                <div className="flex items-start gap-1.5">
                  <p className="italic text-ink-700 text-[13px] leading-snug flex-1">{example.sentence}</p>
                  {example.sentence && (
                    <button
                ...
                {example.translation && (
                  <p className="text-ink-400 text-[11px] leading-snug">{example.translation}</p>
                )}
              </div>
            ))}
          </div>
        ...
        <div className="mb-2">
          ...
          <div className="space-y-1">
            {word.context_sentences.map((cs, index) => (
              <div
                key={index}
                className={`border-l-2 border-amber-300 pl-2.5 rounded-r transition-colors ${disableContextSentenceClick ? '' : 'cursor-pointer hover:bg-parchment-50'}`}
                onClick={() => !disableContextSentenceClick && onSentenceClick && cs.sentence_index !== undefined && onSentenceClick(cs.sentence_index)}
              >
                <div className="flex items-start gap-1.5">
                  <p className="italic text-ink-700 text-[13px] leading-snug flex-1">{cs.sentence}</p>
                  {cs.sentence && (
                    <button
                ...
                {cs.translation && (
                  <p className="text-ink-400 text-[11px] leading-snug">{cs.translation}</p>
                )}
              </div>
            ))}
          </div>
