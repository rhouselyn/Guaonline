...
async def _preprocess_and_run(file_id: str, text: str, source_lang: str, target_lang: str, mode: str, original_text: str, user_id: str = None, tier: str = "free", consumed_quota: int = 0):
    ...
    try:
        ...
        if mode == "translate":
            ...
            response = await gateway.call(user_id, tier, messages, temperature=0.3, max_tokens=8192, request_type="translate")
            ...
            response = await gateway.call(user_id, tier, messages, temperature=0.7, max_tokens=8192, request_type="generate")
            if "choices" in response and len(response["choices"]) > 0:
...
async def translate_text(request: dict, current_user: TokenData = Depends(require_auth)):
    try:
        ...
        response = await gateway.call(current_user.user_id, current_user.tier.value, messages, temperature=0.3, max_tokens=8192, request_type="translate")
...
async def generate_text(request: dict, current_user: TokenData = Depends(require_auth)):
    try:
        ...
        response = await gateway.call(current_user.user_id, current_user.tier.value, messages, temperature=0.7, max_tokens=8192, request_type="generate")
