            ...
            <div className="p-4 border-2 border-dashed border-[#d4c9a8] rounded-md flex flex-wrap gap-2 bg-[#faf8f0]/50 relative">
              <div className="flex flex-wrap gap-2 invisible" aria-hidden="true">
                {data.answer_words.map((_, i) => (
                  <span key={`ph-${i}`} className="px-4 py-2 rounded-md text-sm font-medium">{data.answer_words[i]}</span>
                ))}
              ...
              <div className="absolute inset-0 p-4 flex flex-wrap gap-2 items-start content-start">
                {selectedWords.length === 0 && !answerChecked && (
                  <span className="text-[#8b7e5e] text-sm pointer-events-none">{t.maskedHint || '点击下方选项填入...'}</span>
                )}
                ...
                <motion.button
                  ...
                  className="ml-auto p-2 text-[#d4a853] hover:text-[#d4a853] hover:bg-[#d4a853]/10 rounded-md transition-colors"
          ...
          {answerChecked && (
            ...
            >
              ...
              {isCorrect && isLastExercise && (
                <p className="font-medium mt-3 text-lg text-[#d4a853]" style={{ fontFamily: "'Noto Serif SC', 'Georgia', serif" }}>
