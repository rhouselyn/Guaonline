...
function UnitCompleteStep({ unitNumber, totalUnits, phase, onContinue, onReview, errorCount, hasWrongItems, wrongItemsCount, t, onSkipReview }) {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: -20 }}
      className="max-w-3xl mx-auto relative dotted-bg"
    >
      {hasWrongItems && onSkipReview && (
        <motion.button
          ...
          className="absolute -top-2 left-0 z-10 flex items-center gap-1.5 retro-btn btn-ghost text-xs border-2 border-aged-200 hover:border-aged-300 backdrop-blur-sm"
        >
          <X className="w-3 h-3" />
          {t.skipReview || '不想复习了'}
        </motion.button>
      )}
      <div className="retro-card-highlight border-amber-300 rounded-sm p-12 shadow-retro text-center retro-corners">
        <motion.div
          initial={{ scale: 0 }}
          animate={{ scale: 1 }}
        ...
        {hasWrongItems && (
          <motion.div
            ...
            className="mb-8 p-4 retro-card bg-amber-50 border-amber-200 rounded-sm"
          >
            <div className="flex items-center gap-2 justify-center mb-2">
              <RotateCcw className="w-4 h-4 text-amber-500" />
              <span className="text-ink-700 font-serif font-medium">{t.wrongItemReview || '错题复习'}</span>
            </div>
        ...
        >
          {hasWrongItems ? (
            <motion.button
              ...
              onClick={onReview}
              className="retro-btn btn-primary text-lg flex items-center justify-center gap-2"
            >
